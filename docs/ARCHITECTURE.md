...
11. Whether `runner-engine` should be extracted from `apps/web/src/lib/runner.ts` into its own package — deferred to Milestone 8, when advanced execution modes (iterations, data-driven runs, chaining) give it enough standalone logic to justify the boundary. See "As built — Milestone 7" above.
...
13. Request chaining mechanism (extract a value from one response, feed it into a later request) — deferred to Milestone 8; requires a safe, structured (non-scripted) extraction mechanism to be designed first, since general script execution remains unavailable (see Open Question 3).
