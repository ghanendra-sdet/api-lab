...
## API Contract Testing (Milestone 11)

An OpenAPI document is untrusted input. It is typically fetched from a registry, a colleague, or a public URL, and Milestone 11 treats it with the same suspicion Milestone 6 applies to imported collections.

- **ReDoS is the real risk here, and it was measured, not assumed.** A JSON Schema `pattern` is an attacker-controlled regular expression that would otherwise be handed straight to JavaScript's backtracking regex engine. While choosing the validator for this milestone, the pattern `^(a+)+$` matched against a 31-character non-matching string **blocked the thread for 60 seconds**. In a browser that is the UI thread, and JavaScript offers no timeout and no way to interrupt a running `RegExp.test`, so the only effective mitigation is to never start the match. `redos.ts` statically screens every pattern before it reaches the validator and **removes** unsafe ones from the normalized schema, rejecting over-long sources, patterns that will not compile, and repeated groups with an ambiguous body (a nested quantifier such as `(a+)+`, or an alternation such as `(a|ab)*`). Screening is deliberately conservative and may over-reject; a rejected pattern produces a **warning** stating the check was skipped, never a silent pass and never a spurious failure. Over-rejecting costs a little validation coverage, under-rejecting costs a frozen tab. Milestone 7 flagged the same class of risk for the `matches` assertion operator, where the regex is at least authored by the user themselves; here it arrives inside a third-party document, so the argument for screening is strictly stronger.
- **No code execution, and specifically no schema compilation.** ajv was rejected as the schema validator *because* it compiles schemas into JavaScript and evaluates them with `new Function` — turning an untrusted document into executed code is precisely what the milestone forbids, and it would require `unsafe-eval` in any deployment's CSP. `@cfworker/json-schema` is a pure interpreter with no `new Function` and no `eval`, verified by inspecting its published bundle. There is no `eval`, no `Function` constructor, no dynamic import, and no HTML rendering anywhere in the contract path; violation messages are rendered as React text nodes, never as markup.
- **Bounded by construction.** Specifications over 5MB are rejected before parsing (matching the M6 import limit); at most 2,000 operations are retained (excess is truncated *with a warning*, never silently); schema normalization stops at 64 levels of nesting rather than overflowing the stack; request/response bodies over 2MB are not schema-validated and say so as a warning; violations are capped at 200 so one mismatched 100k-element array cannot allocate 100k violation objects; and the parse cache holds at most 16 entries with LRU eviction.
- **Malformed input degrades, never crashes.** Every entry point returns a typed failure rather than throwing. The recursive normalization walk is wrapped because a pathologically deep document raises a `RangeError`, which Zod's `safeParse` does not catch — the same defence `collection-format`'s `importFile.ts` already uses. An unresolvable `$ref` or a schema the validator rejects becomes a warning-severity violation, not an exception escaping into the request pipeline.
- **YAML is parsed safely.** The `yaml` package's default `parse` is used precisely because it has no equivalent of js-yaml's historical `load`-executes-custom-tags behaviour: no constructor tags, no code execution, no object instantiation from the document, and a built-in alias-expansion budget that bounds "billion laughs" entity expansion. An E2E and a unit test both assert that a `!!js/function` tag does not produce a function.
- **Prototype pollution.** `__proto__`, `constructor`, and `prototype` keys are skipped when copying schemas, components, and extracted path parameters out of an untrusted document, mirroring the existing hardening in `environment-engine` and `runner-engine`.
- **Reports and exports carry no secrets.** The contract report is built only from contract *metadata* — operation paths, JSON paths, schema keywords, and bounded expected/actual type descriptions. It has no field for request headers, authorization values, environment variables, cookies, or raw request/response bodies, so there is no place for a bearer token to travel into an exported file. Credentials are not schema-validated against a contract in the first place. A unit test asserts the serialized export contains none of those terms.
