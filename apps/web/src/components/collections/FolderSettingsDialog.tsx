...
export function FolderSettingsDialog({ collectionId, folder, onClose }: FolderSettingsDialogProps) {
  const updateFolderVariables = useAppStore((s) => s.updateFolderVariables);
  const updateFolderAuth = useAppStore((s) => s.updateFolderAuth);

  const [activeTab, setActiveTab] = useState<"variables" | "auth">("variables");
  const [variables, setVariables] = useState<Variable[]>(folder.variables ?? []);
  ...
  function handleSave() {
    updateFolderVariables(collectionId, folder.id, variables);
    updateFolderAuth(collectionId, folder.id, auth);
    onClose();
  }

  return (
    <Dialog onClose={onClose} titleId="folder-settings-title" className="w-[36rem] max-w-[94vw] p-4">
      <h2
        ...
        {activeTab === "variables" ? (
          <div>
            <p className="mb-3 text-xs text-neutral-500 dark:text-neutral-400">
              Folder variables are shared across all requests inside this folder.
            </p>
            <VariableEditor
              environmentId=""
              variables={variables}
          ...
          <div>
            <p className="mb-3 px-4 text-xs text-neutral-500 dark:text-neutral-400">
              Set the authentication for this folder, or select Inherit to use the collection's authentication.
            </p>
            <AuthFieldsEditor
              auth={auth}
              onChange={setAuth}
